...
function create_topic($data) {
    ...
    try {
        ...
        db_commit();
        if (function_exists('refresh_category_counts')) {
            refresh_category_counts($data['category_id']);
        }
        return $topic_id;
 ...
 * @param int $topic_id Konu ID
 * @return bool Artırıldıysa true
 */
function topic_view_already_counted($topic_id) {
    $topic_id = (int)$topic_id;
    if ($topic_id <= 0) {
        return true;
    }
    $cooldown = defined('TOPIC_VIEW_COOLDOWN') ? (int)TOPIC_VIEW_COOLDOWN : 86400;
    if ($cooldown <= 0) {
        return false;
    }
    if (empty($_SESSION['topic_view_counts']) || !is_array($_SESSION['topic_view_counts'])) {
        return false;
    }
    if (!isset($_SESSION['topic_view_counts'][$topic_id])) {
        return false;
    }
    return (time() - (int)$_SESSION['topic_view_counts'][$topic_id]) < $cooldown;
}

function mark_topic_view_counted($topic_id) {
    if (empty($_SESSION['topic_view_counts']) || !is_array($_SESSION['topic_view_counts'])) {
        $_SESSION['topic_view_counts'] = [];
    }
    $_SESSION['topic_view_counts'][(int)$topic_id] = time();
}
...
function increment_topic_views($topic_id) {
    ...
    if (topic_view_already_counted($topic_id)) {
        @file_put_contents(ROOT_PATH . '/debug-8d0e9d.log', json_encode(['sessionId' => '8d0e9d', 'location' => 'topic.php:increment_topic_views', 'message' => 'view count skipped', 'data' => ['topic_id' => $topic_id, 'session_counts' => array_keys($_SESSION['topic_view_counts'] ?? [])], 'timestamp' => round(microtime(true) * 1000), 'hypothesisId' => 'view-dedup']) . "\n", FILE_APPEND);
        return false;
    }
    $ok = db_execute("UPDATE topics SET views = views + 1 WHERE id = ?", [$topic_id]);
    if ($ok) {
        mark_topic_view_counted($topic_id);
    }
    @file_put_contents(ROOT_PATH . '/debug-8d0e9d.log', json_encode(['sessionId' => '8d0e9d', 'location' => 'topic.php:increment_topic_views', 'message' => 'view count incremented', 'data' => ['topic_id' => $topic_id, 'ok' => (bool)$ok], 'timestamp' => round(microtime(true) * 1000), 'hypothesisId' => 'view-dedup']) . "\n", FILE_APPEND);
    return (bool)$ok;
}
