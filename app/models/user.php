...
function search_users($query, $limit = 10) {
    ...
    return db_query_all(
        "SELECT id, username, avatar FROM users
         WHERE is_banned = 0 AND username LIKE ?
...
function create_user($data) {
    ...
    if ($user_id) {
        ...
        if (function_exists('execute_hooks')) {
            execute_hooks('after_user_register', ['user_id' => $user_id, 'email' => $data['email']]);
        }
        if (function_exists('fire_webhook')) {
            fire_webhook('user.registered', ['user_id' => $user_id]);
        }
    }
