...
function notification_delete_all() {
    require_login();
    if (!is_ajax() || !is_post()) {
        error_response('Geçersiz istek', 400);
        return;
    ...
    $user = current_user();
    db_execute("DELETE FROM notifications WHERE user_id = ?", [$user['id']]);
    success_response(null, 'Tüm bildirimler silindi');
}