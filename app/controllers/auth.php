            ...
            if (!check_rate_limit($client_ip, 'login')) {
                ...
                if (!$user || !verify_password($password, $user['password'])) {
                    ...
                    if ($remember) {
                        ...
                        db_insert(
                            "INSERT INTO remember_tokens (user_id, selector, token_hash, expires_at) VALUES (?, ?, ?, DATE_ADD(NOW(), INTERVAL 30 DAY))",
                            [$user['id'], $selector, $hashed]
                        );
                        $cookie_path = BASE_PATH === '' ? '/' : BASE_PATH . '/';
                        setcookie('remember', $selector . ':' . $token, time() + (86400 * 30), $cookie_path, '', IS_HTTPS, true);
                    }
...
function auth_logout() {
    if (isset($_COOKIE['remember'])) {
        ...
        if (count($parts) === 2) {
            db_execute("DELETE FROM remember_tokens WHERE selector = ?", [$parts[0]]);
        }
        $cookie_path = BASE_PATH === '' ? '/' : BASE_PATH . '/';
        setcookie('remember', '', time() - 3600, $cookie_path, '', IS_HTTPS, true);
    }
    session_destroy();
