...
function create_database_connection() {
    try {
        ...
        if (!file_exists($lock)) {
            require_once dirname(__DIR__) . '/core/detect.php';
            header('Location: ' . rtrim(zunvo_detect_site_url(), '/') . '/install.php');
            exit;
        }
        if (defined('DEBUG_MODE') && DEBUG_MODE === true) {
            die("Veritabanı Bağlantı Hatası: " . $e->getMessage());
        } else {
            die("Veritabanı bağlantısı kurulamadı. Lütfen config/database.php dosyasını kontrol edin.");
        }
    }
}
